...
NEURON {
	POINT_PROCESS Vm_feedback_for_opto
	...
	RANGE v_LP, Up_flag, upward_threshold, downward_threshold, spike_threshold
	...
	USEION ca WRITE cai : used as the I-Clamp command !
}
...
BREAKPOINT {
	...
	: ---------------------------------------------------------------------
	if (v_LP>upward_threshold) { : ==> We are in Up state
	: ---------------------------------------------------------------------
	...
	if (v_LP<downward_threshold) { : ==> We are in the Down state
	: ---------------------------------------------------------------------
	...
	: =====================================================================
	: Triggering and removing stimulation -> *Current-Clamp Command*
	: =====================================================================
	...
	if ((t>up_time_start+delay_vec[i_up]+CMD_delay) && (t<=up_time_start+delay_vec[i_up]+CMD_delay+CMD_dur) && (up_stimulation>=0)) {
	  ...
	  cai = cmd_vec[i_up]
	} else if (up_stimulation>=0) {cai = 0} : this test should control the command only of the up state controls the feedback loop !
	: =====================================================================
	...
	if ((t>down_time_start+delay_vec[i_down]+CMD_delay) && (t<=down_time_start+delay_vec[i_down]+CMD_delay+CMD_dur) && (down_stimulation>=0)) {
	  ...
	  cai = cmd_vec[i_down]
	} else if (down_stimulation>=0) {cai = 0}
	...
	: were not stimulation, we start the stimulation
	: ---------------------------------------------------------------------
	: if ((up_time_start>down_time_start) && (t>up_time_start+delay_vec[i_up]) && (up_stimulation==0) && (t<=up_time_start+delay_vec[i_up]+duration_vec[i_up])) {
	...
	: were not stimulation, we start the stimulation
	: ---------------------------------------------------------------------
	: if ((down_time_start>up_time_start) && (t>down_time_start+delay_vec[i_down]) && (down_stimulation==0) && (t<=down_time_start+delay_vec[i_down]+duration_vec[i_down])) {
